...
if( !$images = $_POST["img"] ){
	...
	$image = ROOT_PA."/wddp/page/".$images;
    ...
    if( !$info ){
		...
		preg_match('|\.(\w+)$|', $image, $ext);
        $ext = strtolower($ext[1]);
		//file_put_contents('a.txt',$ext);
        unset($info);
        // 载入原图
        ...
        $thumbname03 = str_replace("ori", "50", $image);
        $imageFun($thumbImg,$thumbname01,100);
        $imageFun($thumbImg,$thumbname03,100);
        $thumbImg01 = imagecreatetruecolor(200,150);
        imagecopyresampled($thumbImg01,$thumbImg,0,0,$x,$y,200,150,$w,$h);      
        $thumbImg03 = imagecreatetruecolor(50,38);
        imagecopyresampled($thumbImg03,$thumbImg,0,0,$x,$y,50,38,$w,$h);
